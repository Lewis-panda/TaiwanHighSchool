# -*- coding: utf-8 -*-
"""重生「數A4-3 條件機率與貝氏定理」學生講義的章內 SVG。"""

if __name__ == "__main__" and __import__("sys").argv[1:]:
    raise SystemExit("本腳本不接受參數；直接執行即可重生數A4-3章內 SVG。")
...
CHAPTER = os.path.join(ROOT, "content", "數學A", "數A4-3")

FIGURE_OUTPUTS = (
    ("fig_conditioned_space", "數A4-3-條件樣本空間.svg"),
    ("fig_two_way_table", "數A4-3-雙向條件表.svg"),
    ("fig_exclusive_independent", "數A4-3-互斥與獨立.svg"),
    ("fig_chain_tree", "數A4-3-序列乘法樹.svg"),
    ("fig_total_probability", "數A4-3-全機率分割.svg"),
    ("fig_bayes_update", "數A4-3-貝氏面積更新.svg"),
    ("fig_natural_frequencies", "數A4-3-快篩自然頻數.svg"),
    ("fig_frequency_convergence", "數A4-3-客觀機率收斂.svg"),
)


def _save(fig, filename):
    stem, extension = os.path.splitext(filename)
    if extension != ".svg" or not stem.startswith("數A4-3-"):
        raise AssertionError("輸出檔名必須是數A4-3章內 SVG")
...
def fig_conditioned_space():
    ...
    ax.add_patch(Circle((4.1, 3.0), 1.75, facecolor="#dcecff", edgecolor=F.BLUE, lw=2.2, alpha=0.9))
    ax.add_patch(Circle((5.5, 3.0), 1.45, facecolor="#e3f3e7", edgecolor=F.GREEN, lw=2.2, alpha=0.82))
    ax.text(1.0, 4.8, "原樣本空間 S：100 人", fontsize=13, weight="bold")
    ax.text(3.0, 3.95, "A：40 人", color=F.BLUE, fontsize=12)
    ax.text(5.7, 3.8, "B", color=F.GREEN, fontsize=12)
    ax.text(4.45, 2.95, r"$A\cap B$", fontsize=13, weight="bold")
    ax.text(4.42, 2.55, "24 人", fontsize=12)
    ax.text(2.5, 1.15, r"給定 A 後，分母改為 40：  $P(B\mid A)=24/40=0.60$", fontsize=13)
    ax.set_title("條件機率把已知事件 A 當成新的樣本空間", fontsize=16)
    _clean(ax)
    fig.tight_layout()
    return _save(fig, "數A4-3-條件樣本空間.svg")
...
def fig_two_way_table():
    ...
    fig.tight_layout()
    return _save(fig, "數A4-3-雙向條件表.svg")
...
def fig_exclusive_independent():
    ...
    return _save(fig, "數A4-3-互斥與獨立.svg")
...
def fig_chain_tree():
    ...
    fig.tight_layout()
    return _save(fig, "數A4-3-序列乘法樹.svg")
...
def fig_total_probability():
    ...
    fig.tight_layout()
    return _save(fig, "數A4-3-全機率分割.svg")
...
def fig_bayes_update():
    ...
    return _save(fig, "數A4-3-貝氏面積更新.svg")
...
def fig_natural_frequencies():
    ...
    fig.tight_layout()
    return _save(fig, "數A4-3-快篩自然頻數.svg")
...
def fig_frequency_convergence():
    ...
    fig.tight_layout()
    return _save(fig, "數A4-3-客觀機率收斂.svg")
