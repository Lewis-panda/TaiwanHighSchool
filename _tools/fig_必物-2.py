...
def fig_motion_graphs():
    ...
    ax1.set_xlabel("時間 $t$ (s)"); ax1.set_ylabel("位置 $x$ (m)")
    ax1.set_xlim(0, 5); ax1.set_ylim(0, 6); F.clean_grid(ax1)
    ...
    ax2.set_xlabel("時間 $t$ (s)"); ax2.set_ylabel("速度 $v$ (m/s)")
    ax2.set_xlim(0, 5); ax2.set_ylim(0, 9); F.clean_grid(ax2)
...
def fig_free_body():
    ...
    # 接觸面
    ax.add_patch(Rectangle((-2.6, -2.2), 5.2, 0.5, facecolor="#eef1f5",
                           edgecolor=F.INK, hatch="////", lw=1.2))
    ax.plot([-2.6, 2.6], [-1.7, -1.7], color=F.INK, lw=1.6)
    # 物體
    ax.add_patch(Rectangle((-0.7, -1.7), 1.4, 1.4, facecolor="#dbe7ff",
                           edgecolor=F.INK, lw=1.6, zorder=3))
    c = (0.0, -1.0)               # 物體中心（力的作用點）
    ax.add_patch(Circle(c, 0.05, color=F.INK, zorder=6))
    # 四個力
    F.arrow(ax, c, (c[0], c[1] + 2.0), color=F.BLUE);  F.label(ax, (0.0, 1.25), "$N$ 正向力", color=F.BLUE)
    F.arrow(ax, c, (c[0], c[1] - 1.6), color=F.RED);   F.label(ax, (0.0, -2.95), "$mg$ 重力", color=F.RED)
    F.arrow(ax, c, (c[0] + 2.1, c[1]), color=F.GREEN); F.label(ax, (2.55, -1.0), "$T$ 張力", color=F.GREEN, ha="left")
    F.arrow(ax, c, (c[0] - 1.9, c[1]), color=F.AMBER); F.label(ax, (-2.35, -1.0), "$f$ 摩擦力", color=F.AMBER, ha="right")
    ax.set_title("自由體圖（free-body diagram）", fontsize=14)
    ax.set_xlim(-3.4, 3.4); ax.set_ylim(-3.3, 2.0)
...
def fig_incline():
    ...
    ax.add_patch(Polygon([A, B, C], closed=True, facecolor="#eef1f5",
                         edgecolor=F.INK, lw=1.6, hatch="\\\\\\"))
    ...
    # 物體（沿斜面方向的方塊）
    s, tk = 0.95, 0.62
    corners = [c0 - u*s/2 - n*tk/2, c0 + u*s/2 - n*tk/2,
               c0 + u*s/2 + n*tk/2, c0 - u*s/2 + n*tk/2]
    ax.add_patch(Polygon(corners, closed=True, facecolor="#dbe7ff", edgecolor=F.INK, lw=1.6, zorder=3))
    ...
    ax.text(p1[0], p1[1], r"$mg\sin\theta$", color=F.AMBER, fontsize=13, ha="center", va="center", bbox=_bb, zorder=7)
    F.arrow(ax, c0, c0 - n*1.45, color="#6b7280", ls="--")
    p2 = c0 - n*1.55 + np.array([0.62, 0.05])
    ax.text(p2[0], p2[1], r"$mg\cos\theta$", color="#6b7280", fontsize=13, ha="left", va="center", bbox=_bb, zorder=7)
    # 角 θ
    ...
    ax.set_xlim(-4.0, 4.4); ax.set_ylim(-2.4, 3.4)
...
def fig_kepler():
    ...
    # 行星與半徑線
    for t, lab, col in [(np.deg2rad(0), "近日點\n（較快）", F.RED),
                        (np.deg2rad(180), "遠日點\n（較慢）", F.BLUE)]:
        P = np.array([a*np.cos(t), b*np.sin(t)])
        ax.add_patch(Circle(P, 0.13, color=col, zorder=6))
        ax.text(P[0] + (0.4 if t == 0 else -0.4), P[1] + 0.55, lab, color=col,
                ha="left" if t == 0 else "right", va="bottom", fontsize=11)
    ax.text(0, -2.75, "相等時間內，行星與太陽連線掃過的面積相等（等面積定律）",
            ha="center", color=F.INK, fontsize=12)
    ax.set_title("克卜勒第一、第二定律", fontsize=14)
    ax.set_xlim(-4.2, 4.2); ax.set_ylim(-3.2, 3.0)
