...
{\sffamily\bfseries\LARGE\color{HandoutInk}數B4-2 機率與貝氏定理}\par
...
\begin{figure}
\centering
\includegraphics[width=0.88\linewidth,height=\textheight,keepaspectratio,alt={圖 1｜給定事件 A 後，樣本空間縮成 A；交集在 A 內所占比例就是條件機率。}]{assets/數B4-2-條件樣本空間.pdf}
...
\begin{figure}
\centering
\includegraphics[width=0.88\linewidth,height=\textheight,keepaspectratio,alt={圖 2｜雙向表的列、欄總數就是兩種條件方向的分母。}]{assets/數B4-2-雙向條件表.pdf}
...
\begin{figure}
\centering
\includegraphics[width=0.9\linewidth,height=\textheight,keepaspectratio,alt={圖 3｜互斥由交集為零描述；獨立由交集面積等於兩個邊際比例的乘積描述。}]{assets/數B4-2-互斥與獨立.pdf}
...
\subsection{2.2 餘事件與多次獨立試驗}\label{ux9918ux4e8bux4ef6ux8207ux591aux6b21ux7368ux7acbux8a66ux9a57}
...
多個試驗在題目給定「各次結果互不影響」時，一條指定路徑的機率可將各次機率相乘。例如三個元件獨立運作，正常率分別為 \(p_1,p_2,p_3\)，三者都正常的機率為 \(p_1p_2p_3\)。
...
\subsection{完整示範｜獨立發芽試驗與餘事件}\label{ux5b8cux6574ux793aux7bc4ux7368ux7acbux767cux82bdux8a66ux9a57ux8207ux9918ux4e8bux4ef6}

甲、乙兩顆種子的發芽結果獨立，發芽率分別為 \(0.80\) 與 \(0.70\)。求只有甲發芽，以及至少一顆發芽的機率。

「只有甲發芽」對應甲發芽、乙未發芽的同一條路徑：

\[
P(A\cap B')=0.80(1-0.70)=\boxed{0.24}.
\]

至少一顆發芽的餘事件是兩顆都未發芽，因此

\[
P(A\cup B)=1-(1-0.80)(1-0.70)=\boxed{0.94}.
\]
...
\item
  \(A,B\) 獨立，\(P(A)=0.60\)、\(P(B)=0.40\)。求 \(P(A'\cap B)\) 與 \(P(A'\cap B')\)，並以乘積式驗證 \(A',B'\) 獨立。
\item
...
\item
  \(P(A'\cap B)=0.40(0.40)=\boxed{0.16}\)，\(P(A'\cap B')=0.40(0.60)=\boxed{0.24}\)。又 \(P(A')P(B')=0.40(0.60)=0.24=P(A'\cap B')\)，所以 \(A',B'\) 獨立。
\item
...
\begin{figure}
\centering
\includegraphics[width=0.9\linewidth,height=\textheight,keepaspectratio,alt={圖 4｜不放回抽球時，第二次的分母與球數已由第一次結果更新；一條路徑的機率沿枝相乘。}]{assets/數B4-2-序列乘法樹.pdf}
...
\subsection{完整示範｜固定次數中恰有兩次成功}\label{ux5b8cux6574ux793aux7bc4ux56faux5b9aux6b21ux6578ux4e2dux6070ux6709ux5169ux6b21ux6210ux529f}

某次操作成功率為 \(0.40\)，各次結果獨立。重複 \(3\) 次，求恰有 \(2\) 次成功的機率。

三個成功／失敗位置中，可選擇 \(2\) 個位置放成功，共有 \(\binom32=3\) 條互斥路徑。每條路徑的機率都是 \((0.40)^2(0.60)\)，所以

\[
P(X=2)=\binom32(0.40)^2(0.60)=\boxed{0.288}.
\]
...
\item
  四個元件獨立運作，每個正常率皆為 \(0.90\)。求至少一個失效的機率。
\item
...
\begin{figure}
\centering
\includegraphics[width=0.92\linewidth,height=\textheight,keepaspectratio,alt={圖 5｜三個來源先各自產生聯合機率，再把通往瑕疵的互斥路徑相加。}]{assets/數B4-2-全機率分割.pdf}
...
\begin{figure}
\centering
\includegraphics[width=0.88\linewidth,height=\textheight,keepaspectratio,alt={圖 6｜看到瑕疵前以產量比例分配；看到瑕疵後，只在三塊「來源且瑕疵」中重新計算比例。}]{assets/數B4-2-貝氏面積更新.pdf}
...
\begin{figure}
\centering
\includegraphics[width=0.92\linewidth,height=\textheight,keepaspectratio,alt={圖 7｜自然頻數把兩種陽性來源放入同一人數尺度，分母 670 可直接由圖讀出。}]{assets/數B4-2-快篩自然頻數.pdf}
...
\begin{figure}
\centering
\includegraphics[width=0.88\linewidth,height=\textheight,keepaspectratio,alt={圖 8｜固定模型機率下，累積相對次數仍會波動；試驗數增加時，單次結果對比例的影響逐漸變小。}]{assets/數B4-2-客觀機率收斂.pdf}
...
\item
  某種子發芽率為 \(0.70\)，\(4\) 顆種子是否發芽彼此獨立。求至少 \(3\) 顆發芽的機率。
\end{enumerate}
...
\item
  某公司過去資料中，甲供應商 \(500\) 件產品有 \(10\) 件瑕疵，乙供應商 \(300\) 件產品有 \(18\) 件瑕疵。新一批產品來自甲、乙的比例為 \(0.60,0.40\)，並以過去相對次數估計各供應商的瑕疵率。
\end{enumerate}
...
至少 \(3\) 顆發芽包含恰好 \(3\) 顆與 \(4\) 顆都發芽兩個互斥事件：

\[
\begin{aligned}
P(X\ge3)
&=\binom43(0.70)^3(0.30)+(0.70)^4\\
&=0.4116+0.2401\\
&=\boxed{0.6517}.
\end{aligned}
\]

二項分別對應「從四個位置選一個未發芽」與「四顆全部發芽」。
...
\[
P(D\mid A)=\frac{10}{500}=0.02,
\qquad
P(D\mid B)=\frac{18}{300}=0.06.
\]

甲、乙兩來源形成分割，因此

\[
P(D)=0.60(0.02)+0.40(0.06)=0.012+0.024=\boxed{0.036}.
\]
...
\[
P(B\mid D)
=\frac{0.40(0.06)}{0.036}
=\boxed{\frac23}.
\]

第 (1) 題把實際資料轉成客觀機率，再用全機率合併來源；第 (2) 題把瑕疵當成新證據，以貝氏定理更新來源機率。
