...
把有理數 $\dfrac{p}{q}$ 做\textbf{長除法}，每一步的餘數只能是 $0,1,2,\ldots,q-1$ 這 $q$ 種。若某次餘數為 $0$，除法結束，得到\textbf{有限小數}；若一直除不盡，餘數永不為 $0$，於是要排出無限多個餘數，但餘數的「種類」至多 $q$ 種，依\textbf{鴿籠原理}\textbf{必有一個餘數重複出現}；某個餘數一旦重現，後續的除法步驟就會原封不動地重來，從該處起小數開始\textbf{週期性循環}（循環節長度不超過 $q-1$）。反過來，任何循環小數也都能還原成分數。

\begin{補充框}{用到的工具：鴿籠原理（pigeonhole principle）}
若把 $n+1$ 個（含以上）物件放進 $n$ 個盒子，則\textbf{至少有一個盒子裝了兩個（含以上）物件}。敘述直觀，卻是「存在性」論證的常用工具：上面「餘數一定會重複」、生活中「任取 $13$ 人必有兩人同月份生日」都是它的應用。在這裡，它保證了長除法的餘數遲早撞號，循環因此非發生不可。
\end{補充框}
...
三段同除以係數時須留意正負：除以\textbf{正數}時不等號方向不變，除以\textbf{負數}時三段的不等號方向\textbf{都要反轉}。以 $|3-2x|\le5$ 為例：先夾成 $-5\le 3-2x\le5$，各段減 $3$ 得 $-8\le -2x\le2$，再各段同除以 $-2$（三個不等號都翻向）得 $4\ge x\ge-1$，即 $-1\le x\le4$。若不放心除負數這一步，可先用 $|3-2x|=|2x-3|$ 把首項係數化為正，再依正係數的作法夾，結果相同。
...
對任意實數 $a,b$：
$$\boxed{\;|a+b|\le|a|+|b|\;}$$
等號成立 $\iff a,b$ \textbf{同號，或其中一個為 $0$}。它的意思是：先相加再取絕對值，不會比先各取絕對值再相加更大。名稱「三角」來自向量版本——把 $a,b$ 看成首尾相接的兩個向量，$a+b$ 是合向量，三者圍成一個三角形，而\textbf{三角形兩邊長之和不小於第三邊}就是 $|a|+|b|\ge|a+b|$。當 $a,b$ 同方向（同號）時三角形退化成一直線，兩邊攤平剛好接成第三邊，此時等號成立。
\end{補充框}

\fig{d11-triangle-ineq}{0.92}{圖 1-5：三角不等式的幾何來源。左——$a,b$ 方向不同時，$\vec a$、$\vec b$、$\vec a+\vec b$ 圍成三角形，第三邊 $|\vec a+\vec b|$ 小於另兩邊長之和 $|\vec a|+|\vec b|$；右——$a,b$ 同方向時三角形退化成一直線，$|\vec a+\vec b|=|\vec a|+|\vec b|$，等號成立。}
