...
\begin{補充框}{算幾不等式的兩種極值：和定積最大、積定和最小}
算幾不等式 $\dfrac{a+b}{2}\ge\sqrt{ab}$ 把「和」與「積」綁在一起，因此固定其中一個，另一個就有確定的界，並在 $a=b$ 時取到。兩種方向恰好互為對稱。

\textbf{(1) 積定，和最小。}\ 若兩正數的乘積 $ab$ 為定值，則 $\sqrt{ab}$ 是定值，於是
$$a+b\ge2\sqrt{ab}=\text{（定值）},$$
和 $a+b$ 有最小值，在 $a=b$ 時取到。

\textbf{(2) 和定，積最大。}\ 反過來，若兩正數的和 $a+b$ 為定值，則 $\dfrac{a+b}{2}$ 是定值，兩邊平方得
$$ab\le\left(\frac{a+b}{2}\right)^2=\text{（定值）},$$
積 $ab$ 有最大值，同樣在 $a=b$ 時取到。

兩者可用一句口訣記住：\textbf{「積定和最小、和定積最大」}。其共同精神是「在受限制的條件下，兩數越接近（最終相等）越接近最佳值」——這正是等號條件 $a=b$ 的意義。
\end{補充框}

\fig{d13-amgm-sumfixed}{0.66}{圖 3-3：「和定積最大」的幾何讀法。把 $a,b$ 看成長方形的長與寬，則 $a+b$ 為半周長、$ab$ 為面積。三個長方形的半周長都固定為 $a+b=10$，面積卻不同（$9$、$21$、$25$）；邊長越接近，面積越大，在正方形 $a=b=5$ 時面積最大 $=25=\left(\frac{a+b}{2}\right)^2$。這正是 $ab\le\left(\frac{a+b}{2}\right)^2$ 的圖像化。}
...
\textbf{(2) 求極值的用途。}\ 如上方補充框所述，算幾不等式可同時用於兩類極值：積為定值時和有最小值、和為定值時積有最大值。求最小值一類的關鍵是讓兩個正項\textbf{相乘剛好把變數消成常數}，它們的和才有確定的下界；求最大值一類則是把和固定後平方放大為積的上界。
...
\fig{d13-log}{0.62}{圖 3-4：指數與對數互為反函數。$y=2^x$（藍）與 $y=\log_2 x$（紅）對直線 $y=x$（虛線）對稱；前者過 $(0,1)$，後者過 $(1,0)$，座標恰好對調。}
...
\begin{補充框}{用對數估計位數（$N>1$）與小數位（$0<N<1$）}
首數決定數量級，因此無論 $N$ 大於 $1$（問整數位數）或介於 $0$ 與 $1$ 之間（問小數位），都靠首數判定，只是方向相反。

\textbf{情形一（$N>1$，問整數位數）。}\ 一個正整數 $N$ 的位數 $=\lfloor\log N\rfloor+1$，其中 $\lfloor\ \cdot\ \rfloor$ 為高斯符號（取不超過該數的最大整數）。\\
道理：$N$ 為 $d$ 位數 $\iff 10^{d-1}\le N<10^{d}\iff d-1\le\log N<d$，因此 $d=\lfloor\log N\rfloor+1$，即\textbf{首數（非負）加 $1$ 就是位數}。

\textbf{情形二（$0<N<1$，問小數點後第幾位首現非零數字）。}\ 此時 $\log N<0$，首數為負整數。若首數為 $-k$（即 $\log N=-k+\text{尾數}$，$k$ 為正整數），則
$$10^{-k}\le N<10^{-k+1},$$
故 $N$ 的小數點後\textbf{第 $k$ 位}才出現第一個非零數字。\\
道理：$N$ 介於 $10^{-k}$（即 $0.0\cdots01$，小數點後第 $k$ 位為 $1$）與 $10^{-k+1}$ 之間，數量級由負首數 $-k$ 決定。\textbf{負首數的絕對值，就是第一個非零數字所在的小數位。}

兩種情形對稱：首數 $\ge0$ 用情形一數整數位，首數 $<0$ 用情形二數小數位。
...
\item \textbf{算幾不等式}：$\dfrac{a+b}{2}\ge\sqrt{ab}$（$a,b>0$），等號 $\iff a=b$，可由 $(\sqrt a-\sqrt b)^2\ge0$ 證得。求極值口訣「積定和最小、和定積最大」。
...
\item \textbf{首數／尾數}：$\log N=n+\log a$（$N=a\times10^n$）。$N>1$ 時整數位數 $=\lfloor\log N\rfloor+1=$ 首數 $+1$；$0<N<1$ 時首數為 $-k$，則小數點後\textbf{第 $k$ 位}才首現非零數字（兩種數對稱判定）。
