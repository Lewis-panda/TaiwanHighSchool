...
\fig{d21-ssa-cases}{0.96}{圖 1-4：SSA 模糊情況。固定角 $A$ 與邊 $b$，以 $C$ 為圓心、半徑 $a$ 畫弧（紅），弧與底邊的交點（綠）即三角形解。$a<b\sin A$（短於高 $h$）搆不到底邊故無解；$a=b\sin A$ 恰碰底邊一點（直角三角形）為一解；$b\sin A<a<b$ 弧穿過底邊兩點為兩解。}
...
兩條相交直線斜率分別為 $m_1,m_2$（\textbf{兩者皆存在}），它們夾角 $\theta$ 的正切可由斜率直接算（推導用「兩傾斜角相減」加上高二的正切差角公式，這裡先給結果使用）：
...
\textbf{注意：其中一條是鉛直線時不能套此公式。}上式有個前提，兩條直線的斜率都要存在。若有一條是\textbf{鉛直線（傾斜角 $90^\circ$、斜率不存在）}，$m$ 代不進去，這時改用更根本的「\textbf{傾斜角相減}」：兩直線傾斜角為 $\alpha_1,\alpha_2$，夾角的銳角就是 $|\alpha_1-\alpha_2|$ 與 $180^\circ-|\alpha_1-\alpha_2|$ 中不超過 $90^\circ$ 的那個；鉛直線的傾斜角直接記成 $90^\circ$。其實「傾斜角相減」才是夾角的本質，斜率公式只是它在「兩斜率都存在」時的代數化身。
...
\fig{d21-elevation-depression}{0.62}{圖 1-5：仰角與俯角。觀測者由\textbf{水平線}量起：目標在上方、視線上抬的夾角是仰角；目標在下方、視線下俯的夾角是俯角。兩者都不是從鉛直線量。}
...
\fig{d21-polar}{0.62}{圖 1-6：極坐標與直角坐標互換。點 $P$ 既可記為直角坐標 $(x,y)$，也可記為極坐標 $(r,\theta)$，兩者由 $x=r\cos\theta$、$y=r\sin\theta$ 相連。}
