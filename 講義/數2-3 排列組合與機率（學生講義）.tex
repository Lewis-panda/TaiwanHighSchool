...
\fig{d23-venn3}{0.98}{圖 3-2：三集合取捨原理。\textbf{左}——三圓相交把聯集切成七塊，逐塊追蹤「加單、減雙、加三」後被淨數的次數：只在單一圓的區塊（如 $+1$）淨數 1 次、恰落在兩圓的區塊（$2-1$）淨數 1 次、三圓共有的區塊（$3-3+1$）也淨數 1 次——每塊都恰好 1 次，這就是公式各項符號的由來。\textbf{右}——以「$1\sim100$ 中 $2,3,5$ 的倍數」（例 3-3b）實填各區個數，七塊相加 $=74$。}
...
\fig{d23-tree}{0.62}{圖 3-3：樹狀圖。以「2 種上衣搭 3 種褲子」為例，從起點先分出 2 種上衣，每種上衣再分出 3 種褲子，走到底共 $2\times3=6$ 條路徑，對應 6 種搭配。}
...
\medskip
前面排的都是「排成一列」（\textbf{直線排列}），位置有頭有尾。但若把 $n$ 個相異物\textbf{圍成一圈}（如圍圓桌而坐），整桌一起旋轉後每個人的左右鄰居都沒變，應算\textbf{同一種}。這種「只看相對位置」的排列叫\textbf{環狀排列}，其方法數為
$$\boxed{\,(n-1)!\,}$$
\textbf{理由}：先當直線排列有 $n!$ 種；但圍成圈後，每一種圈排法都能整體旋轉到 $n$ 個位置而看起來相同，重複算了 $n$ 次，故 $\dfrac{n!}{n}=(n-1)!$。等價說法是「\textbf{固定其中一人}當參考點（破除旋轉自由度），其餘 $n-1$ 人再直線排列」。圍圈時「相鄰用綁綑」等招式完全通用（如 $n$ 人圍圓桌、某兩人相鄰：兩人綁成一綑視為 $n-1$ 個單位圍圈 $(n-2)!$，綑內互換 $2!$）。

\fig{d23-circular}{0.95}{圖 3-4：環狀排列。以 4 人圍圓桌為例，同一種坐法整體旋轉到 4 個位置看起來「都不同」，但左右鄰居完全沒變、其實算同一種，被重複數了 4 次，故方法數 $=\tfrac{4!}{4}=(4-1)!=3!=6$。}
...
\fig{d23-perm-comb}{0.92}{圖 3-5：以「從 $\{A,B,C\}$ 取 2 個」對照。左——排列 $P^3_2=6$（順序不同算不同，如 $AB$、$BA$ 視為兩種）；右——組合 $C^3_2=3$（順序不同算同一組，$\{A,B\}$ 只算一種）。每個組合都對應 $2!=2$ 個排列，故 $C^3_2=P^3_2/2!=3$。}
...
\fig{d23-probability}{0.96}{圖 3-6：左——擲一顆公正骰子，樣本空間 $S=\{1,2,3,4,5,6\}$（六面等可能），事件「出現偶數」$A=\{2,4,6\}$，故 $P(A)=\tfrac{3}{6}=\tfrac12$。右——期望值是「以機率為權重的平均」（見 C 段）：各結果的數值乘上各自機率再相加。}
...
\item \textbf{排列（在意順序）}：$P^n_r=\dfrac{n!}{(n-r)!}=n(n-1)\cdots(n-r+1)$，全排列 $P^n_n=n!$（規定 $0!=1$）；\textbf{相同物排列} $\dfrac{n!}{n_1!\,n_2!\cdots n_k!}$；\textbf{環狀排列} $(n-1)!$（圍圈、旋轉視為同一種）。
...
\end{itemize}
\textbf{常用招式}：「至少」型用餘事件、「不相鄰」用反面扣除、「相鄰」用綁綑法、圍圈用環狀 $(n-1)!$、分子分母數法要一致、平均分成地位相同的組要再除階乘。
\end{補充框}
