...
\fig{d24-stddev}{0.92}{圖 4-1：兩組資料平均數都是 $50$，但甲組擠在平均附近（標準差約 $1.4$），乙組散得很開（標準差約 $14.1$）。標準差越大，資料越分散。}
...
\fig{d24-standardize}{0.92}{圖 4-2：原始分布平均 $70$、標準差 $8$，原始分數 $86$ 在平均上方 $2$ 個標準差處；標準化後整個分布平移縮放，原來的 $86$ 對應到 $z=+2$。}
...
\fig{d24-scatter}{0.96}{圖 4-3：散布圖的三種走向。左——正相關（左下到右上）；中——負相關（左上到右下）；右——近乎無相關（看不出隨 $x$ 的趨勢）。}
...
\fig{d24-bestfit}{0.62}{圖 4-4：橘色虛線是各點的殘差（資料點到直線的鉛直距離）。最適直線就是讓這些殘差「平方後加起來」最小的那條，且必通過平均點。}
