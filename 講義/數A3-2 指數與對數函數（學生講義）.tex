...
由換底公式可直接得到兩個好用的推論：
$$\log_a b=\frac{1}{\log_b a}\qquad(\text{倒數律，把 }c\text{ 選成 }b),\qquad \log_{a^{n}}b^{m}=\frac{m}{n}\log_a b.$$
...
\fig{d32-ineq-flip}{0.96}{圖 2-5：底數 $<1$ 時為什麼不等號要反向。左——指數 $y=a^x$（$0<a<1$）遞減，雖然 $f<g$，但 $a^f>a^g$；右——對數 $y=\log_a x$（$0<a<1$）遞減，雖然 $A<B$，但 $\log_a A>\log_a B$。}
...
\fig{d32-growth-decay}{0.96}{圖 2-3：指數模型。左——複利（指數，曲線）期數一大就遠遠把單利（線性，直線）甩開；右——半衰期衰退，每過一個半衰期 $T$ 數量減半。}
...
\fig{d32-log-scale}{0.96}{圖 2-4：同一條 $y=10^x$。左——線性軸上小值全被壓在底部看不見；右——改用對數軸，每格 $\times10$，跨數量級也看得清，且取 $\log$ 後成等差（直線）。}
