...
寫成坐標：$P=\left(\dfrac{n x_1+m x_2}{m+n},\ \dfrac{n y_1+m y_2}{m+n}\right)$。當 $m=n$ 時退化為中點公式。這條公式不是背的：由 $\overline{AP}:\overline{PB}=m:n$ 知 $\overrightarrow{AP}=\dfrac{m}{m+n}\overrightarrow{AB}$，於是 $\overrightarrow{OP}=\overrightarrow{OA}+\overrightarrow{AP}=\overrightarrow{OA}+\dfrac{m}{m+n}(\overrightarrow{OB}-\overrightarrow{OA})$；併項後 $\overrightarrow{OA}$ 的係數為 $\dfrac{n}{m+n}$，即得上式，「$m$ 配 $B$」的交叉配法自然就冒出來了。
...
\textbf{平行判定為什麼是 $a_1b_2-a_2b_1=0$。}「$\vec a\parallel\vec b$」表示分量成比例：$b_1=ka_1$、$b_2=ka_2$。把兩式交叉相乘消去 $k$，得 $a_1b_2=a_2b_1$，即 $a_1b_2-a_2b_1=0$。反之，若此式成立（且 $\vec a\ne\vec 0$），則分量比例一致，可找到 $k$ 使 $\vec b=k\vec a$。兩者完全等價——這只是把「比例式」改寫成「不含分母的乘法式」，順便避開 $a_1$ 或 $a_2$ 為 $0$ 時不能相除的麻煩。這個量正是 3-4 的二階行列式。

\fig{d33-perp-parallel}{0.92}{圖 3-4：兩種判定剛好相反。左——\textbf{垂直}用\textbf{內積}：$\vec a\perp\vec b\iff\vec a\cdot\vec b=0$（圖中直角記號）；右——\textbf{平行}用\textbf{行列式}：$\vec a\parallel\vec b\iff a_1b_2-a_2b_1=0$（兩向量落在同一條直線上，同向或反向）。}
...
\fig{d33-projection}{0.60}{圖 3-5：$\vec b$ 在 $\vec a$ 上的正射影。從 $\vec b$ 的尖端對 $\vec a$ 所在直線作垂線，落點與原點之間沿 $\vec a$ 方向的那支紅色向量，即正射影向量 $\left(\dfrac{\vec a\cdot\vec b}{|\vec a|^2}\right)\vec a$。}
...
\fig{d33-determinant-area}{0.58}{圖 3-6：以 $\vec a=(x_1,y_1)$、$\vec b=(x_2,y_2)$ 為鄰邊的平行四邊形，其面積等於二階行列式的絕對值 $|x_1y_2-x_2y_1|$。}
