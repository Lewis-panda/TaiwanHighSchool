...
\textbf{兩相異平面 $E_1$、$E_2$}：\textbf{平行}（無交點）、或\textbf{相交於一條直線}。（注意：兩相異平面若相交，交集是\textbf{一條直線}，不是一個點。想像兩張紙互相切過。）
...
\fig{d34-three-perp}{0.62}{圖 4-1：三垂線定理。$PA$ 垂直於地面，$A$ 是 $P$ 的正射影；在地面上 $AB\perp L$，則斜拉線 $PB$ 也垂直於 $L$。$\overline{PB}$ 即點 $P$ 到直線 $L$ 的距離。}
...
\fig{d34-coords}{0.6}{圖 4-2：空間直角坐標系。三軸兩兩垂直，點 $P(a,b,c)$ 由三個坐標定位；虛線顯示 $P$ 到各坐標平面的投影。}
...
\fig{d34-cross-product}{0.6}{圖 4-3：外積 $\vec a\times\vec b$ 垂直於 $\vec a,\vec b$ 所在的平面（右手定則決定朝向），其長度等於以 $\vec a,\vec b$ 為鄰邊的平行四邊形面積。}
...
\textbf{薩魯斯法（Sarrus，對角線法）}是另一種算法：把前兩行抄到右邊，三條「右下方向」對角線的乘積相加、三條「左下方向」對角線的乘積相減。\textbf{此法只對三階有效}，更高階不能用。
\end{補充框}
...
\fig{d34-parallelepiped}{0.6}{圖 4-4：以 $\vec a,\vec b,\vec c$ 為鄰邊的平行六面體。底面是 $\vec a,\vec b$ 撐的平行四邊形（面積 $=|\vec a\times\vec b|$），高是 $\vec c$ 在法向量 $\vec a\times\vec b$ 方向上的投影長。}
...
\textbf{空間概念}
\begin{itemize}\setlength{\itemsep}{1pt}
\item 兩直線：平行 / 相交 / \textbf{歪斜}（不共面，平面沒有）。兩平面：平行 / 交於\textbf{一直線}。線與面：在面內 / 平行 / 交於一點。
