...
兩平面相交時，交集是一條直線，那就要會把\textbf{這條交線寫出來}。交線同時躺在兩個平面上，所以它\textbf{同時垂直於兩個法向量} $\vec n_1$、$\vec n_2$——這正是外積的拿手好戲。

\fig{d41-two-planes}{0.60}{圖 2：兩相交平面的交線 $L$。交線同時順著兩平面、同時垂直 $\vec n_1$ 與 $\vec n_2$，因此方向就是外積 $\vec n_1\times\vec n_2$。}

\begin{補充框}{兩相交平面的交線怎麼求}
兩平面 $E_1$、$E_2$（法向量 $\vec n_1$、$\vec n_2$，$\vec n_1\not\parallel\vec n_2$）相交於一直線 $L$。
\begin{itemize}\setlength{\itemsep}{2pt}
\item \textbf{交線方向}：$\boxed{\;\vec d=\vec n_1\times\vec n_2\;}$（同時垂直兩法向量，故同時順著兩平面）。
\item \textbf{交線上一點}：解聯立 $E_1=0$、$E_2=0$，\textbf{指定一個座標}（例如令 $z=0$）解出另兩個座標，得交線上一個具體點。
\item 有了「一點 ＋ 方向」就能寫出交線的參數式或比例式（見 1-2）。
\end{itemize}
\textbf{取點時的小陷阱：}「令某座標 ＝ 常數」要挑一個\textbf{真能解出唯一交點}的座標。若令 $z=0$ 後聯立無解，改令 $x=0$ 或 $y=0$ 試。原則上方向 $\vec d$ 哪個分量不為 0，就固定「對應那一軸」的座標來求點。
\end{補充框}
...
\fig{d41-dihedral-angle}{0.62}{圖 3：兩平面的夾角 $\theta$ 等於兩法向量 $\vec n_1$、$\vec n_2$ 的夾角（取不超過 $90^\circ$ 的那一個）。}
...
\fig{d41-point-to-plane}{0.60}{圖 4：點 $P_0$ 到平面的距離，是把連線投影到法向量 $\vec n$ 方向後的長度（垂直高度）。}
...
\fig{d41-line-param}{0.60}{圖 5：直線上的動點 $P$ 由「定點 $A$ ＋ $t$ 倍方向向量 $\vec d$」給出。$t=0$ 時回到 $A$，掃過 $t$ 就掃出整條線。}
...
\fig{d41-line-plane}{0.92}{圖 6：直線與平面的三種關係。$\vec d\cdot\vec n\ne0$ 時相交於一點；$\vec d\cdot\vec n=0$ 時平行或內含，再看定點在不在平面上。}
...
\textbf{一個常用特例（直線垂直於平面）：}在「相交於一點」裡，當 $\vec d\parallel\vec n$（方向向量與法向量平行）時，直線\textbf{垂直於平面}、戳得最正。這是「兩平面垂直 $\iff$ 法向量垂直」的對偶——對直線與平面而言，\textbf{方向向量平行法向量 $\iff$ 線面垂直}。求垂足、作正射影時都靠它。
...
\fig{d41-two-lines}{0.92}{圖 7：空間中兩直線的三種位置關係。平行與相交兩線共平面；歪斜是空間特有——兩線方向不同、又錯開在不同高度，既不相交也不共平面。}
...
\fig{d41-skew-lines}{0.62}{圖 8：兩歪斜線 $L_1$、$L_2$ 與它們的公垂線段。公垂方向同時垂直 $\vec d_1$、$\vec d_2$，正是外積 $\vec d_1\times\vec d_2$；該線段長度即兩線距離。}
