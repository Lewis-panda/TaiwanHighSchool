...
\item \textbf{零矩陣（zero matrix）$O$}：所有元素都是 $0$ 的矩陣，是矩陣加法的「$0$」，滿足 $A+O=A$。
\end{itemize}
...
\item \textbf{鏡射（reflection）}對 $x$ 軸、$y$ 軸、直線 $y=x$、直線 $y=-x$：
...
其中對 $y=x$ 鏡射是把坐標對調 $(x,y)\mapsto(y,x)$、對 $y=-x$ 鏡射是對調再變號 $(x,y)\mapsto(-y,-x)$。
...
\fig{d42-four-transforms}{0.98}{圖 2-3：四種標準變換對同一個單位方格（虛線藍）的作用對比。伸縮把方格沿坐標軸拉長；推移（shear）把方格推成平行四邊形；旋轉整體轉一個角度；鏡射把方格翻到 $x$ 軸另一側。紅色箭頭為兩基底 $(1,0)$、$(0,1)$ 的像，即矩陣的兩行。}

\fig{d42-rotation-reflection}{0.92}{圖 2-4：旋轉（繞原點逆時針轉 $\theta$，左）與兩條對角線鏡射（對 $y=x$、對 $y=-x$，右）。旋轉的 $\det=1$、鏡射的 $\det=-1$（負號代表把定向翻面），兩者都不改變面積。}
...
$$R_\theta=\begin{bmatrix} \cos\theta & -\sin\theta\\ \sin\theta & \cos\theta \end{bmatrix}.$$
那些三角函數不是憑空出現的，它們就是\textbf{兩個基底轉動後的坐標}，是單位圓的定義直接搬過來的。鏡射用完全相同的方法：對 $y=x$ 鏡射，$\vec e_1=(1,0)\mapsto(0,1)$、$\vec e_2=(0,1)\mapsto(1,0)$，得 $\begin{bmatrix} 0 & 1\\ 1 & 0 \end{bmatrix}$；對 $y=-x$ 鏡射，$\vec e_1\mapsto(0,-1)$、$\vec e_2\mapsto(-1,0)$，得 $\begin{bmatrix} 0 & -1\\ -1 & 0 \end{bmatrix}$。
...
\fig{d42-transition}{0.86}{圖 2-5：二階轉移方陣的演化。反覆作用 $\vec v_n=P^n\vec v_0$，分布逐步收斂到一個固定的穩定分布 $\vec v_*$，且與起點無關。}
