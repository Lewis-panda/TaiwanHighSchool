...
上拋全程都是 $a=-g$ 的等加速度運動，上升、最高點、下降是同一條 $x(t)$ 的不同階段，不必另記一套公式。一張從 $+v_0$ 線性降到負值的 $v\text{–}t$ 圖即可說明全部：過零點對應最高點，圖形對稱於該點。

\fig{si2-freefall}{0.5}{圖 2-2：鉛直上拋的對稱性（$v_0=20\ \text{m/s}$，$g=10\ \text{m/s}^2$）。最高點 $v=0$，但 $a=-g$ 仍向下；同一高度在 $t=1\ \text{s}$（上升）與 $t=3\ \text{s}$（下降）各被經過一次，速率相同、方向相反；$t=0\ \text{s}$ 拋出（$v=20$ 向上）與 $t=4\ \text{s}$ 回到拋出點（$v=20$ 向下）速率相同。}
...
\fig{si2-relative}{0.78}{圖 2-3：一維相對速度。A 車 $+20\ \text{m/s}$、B 車 $+12\ \text{m/s}$ 同向行駛（向右為正）。A 上的人看 B：$v_{B/A}=12-20=-8\ \text{m/s}$（B 相對 A 向左退）；B 上的人看 A：$v_{A/B}=20-12=+8\ \text{m/s}$。兩者等大反向。}
...
依同樣的邏輯再取一次變化率（加速度是速度的變化率），得
$$a_{B/A} = a_B - a_A.$$
所以加速度也能相對，同樣是相減。其中有一個重要特例：若 A 本身\textbf{等速度}運動（$a_A=0$），則 $a_{B/A}=a_B$——\textbf{B 的加速度在 A 看來，和在地面看來完全相同}。也就是說，速度會隨參考系改變，但只要參考系彼此等速度，加速度就不變。
...
\item \textbf{瞬時＝圖的斜率}：$x\text{–}t$ 斜率＝瞬時速度；$v\text{–}t$ 斜率＝瞬時加速度、\textbf{面積＝位移}。
...
\item \textbf{一維相對速度}：$v_{B/A}=v_B-v_A$（先定正方向、帶正負號）；同向相減、反向相加。等速參考系間加速度不變，故力學定律形式相同（伽利略相對性）。
\end{itemize}
\end{補充框}
