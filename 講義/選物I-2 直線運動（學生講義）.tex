...
\fig{si2-freefall}{0.5}{圖 2-3：鉛直上拋的對稱性（$v_0=20\ \text{m/s}$，$g=10\ \text{m/s}^2$）。最高點 $v=0$，但 $a=-g$ 仍向下；同一高度在 $t=1\ \text{s}$（上升）與 $t=3\ \text{s}$（下降）各被經過一次，速率相同、方向相反；$t=0\ \text{s}$ 拋出（$v=20$ 向上）與 $t=4\ \text{s}$ 回到拋出點（$v=20$ 向下）速率相同。}
...
\fig{si2-relative}{0.78}{圖 2-4：一維相對速度。A 車 $+20\ \text{m/s}$、B 車 $+12\ \text{m/s}$ 同向行駛（向右為正）。A 上的人看 B：$v_{B/A}=12-20=-8\ \text{m/s}$（B 相對 A 向左退）；B 上的人看 A：$v_{A/B}=20-12=+8\ \text{m/s}$。兩者等大反向。}
