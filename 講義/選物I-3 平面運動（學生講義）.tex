...
\fig{si3-boat-triangle}{0.92}{圖 3-5：河中行船的速度向量三角形（船速、水流速、合速度頭尾相接，對地＝船對水＋水對地）。左——船頭垂直對岸（船速 $\perp$ 河岸），合速度斜指下游、渡河最快；右——船頭朝上游偏 $\theta$，使船速的沿河岸分量抵消水流，合速度恰垂直對岸。兩圖均取 $v=5$、$u=3$（同例題），右圖 $\sin\theta=u/v=3/5$、$\theta=37^\circ$。}
