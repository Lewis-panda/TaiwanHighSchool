...
依運動獨立性，分兩個方向處理。取拋出點為原點、水平向右為 $x$ 正向、鉛直向下為 $y$ 正向，初速為 $v_0$（水平）。其中 $g$ 為\textbf{重力加速度（gravitational acceleration）}——地表附近自由下落的物體，每秒速度增加約 $9.8\ \text{m/s}$，計算時多取 $g\approx10\ \text{m/s}^2$，與自由落體所用的 $g$ 相同：
...
\fig{si3-monkey}{0.74}{圖 3-5：「猴子與獵人」。灰色虛線為無重力時的直線瞄準路徑，藍色為子彈加上重力後的實際拋物線，紅色為目標放手後的自由落線。在同一時刻，子彈相對瞄準線下墜的高度與目標下墜的高度都是 $\tfrac12 g t^2$（等高的虛線），故兩者一路等高、必在橘圈處相撞。}
...
\subsection*{\sffamily B.\ 一般情形：兩物體沿不同方向運動}

在套用到行船、飛機之前，先看最一般的情形——這也是 $\vec v_{B/A}=\vec v_B-\vec v_A$ 真正的用法：A、B 兩物體在平面上各自沿\textbf{不同方向}運動（不必垂直、也不必同一條線），要算「站在 A 上看 B 怎麼動」，就把兩個對地速度向量\textbf{相減}，一樣分 $x$、$y$ 分量各自做，再合成大小與方向。

最經典的例子是\textbf{雨中行人}：雨鉛直落下（對地速度 $\vec v_{\text{雨}}$ 向下），人水平前進（對地速度 $\vec v_{\text{人}}$ 向前）。站在人身上看雨，
$$\vec v_{\text{雨/人}}=\vec v_{\text{雨}}-\vec v_{\text{人}},$$
把 $-\vec v_{\text{人}}$（與前進方向相反、水平向後）接在 $\vec v_{\text{雨}}$ 之後，合成的相對速度\textbf{斜向前下方}。這就是為什麼走得越快，雨越像是迎面斜打過來，傘須往前傾才擋得住。例如雨以 $4\ \text{m/s}$ 鉛直落下、人以 $3\ \text{m/s}$ 水平前進時，相對速度大小為 $\sqrt{3^2+4^2}=5\ \text{m/s}$，與鉛直方向夾角 $\tan\phi=\tfrac34$（約 $37^\circ$），故傘朝前傾約 $37^\circ$。接下來的行船、飛機，都是這個「向量相減」的特例——只是那裡兩個已知向量恰好互相垂直，算起來更乾淨。

\fig{si3-relvel}{0.86}{圖 3-6：一般平面相對速度。左——地面看到的兩速度（雨鉛直落、人水平走）。右——把兩者相減：$\vec v_{\text{雨}}$ 接上 $-\vec v_{\text{人}}$，第三邊 $\vec v_{\text{雨/人}}$ 即人眼中的雨速，斜向迎面而來。}

\subsection*{\sffamily C.\ 河中行船：兩種典型問法}
...
\fig{si3-boat-triangle}{0.92}{圖 3-7：河中行船的速度向量三角形（船速、水流速、合速度頭尾相接，對地＝船對水＋水對地）。左——船頭垂直對岸（船速 $\perp$ 河岸），合速度斜指下游、渡河最快；右——船頭朝上游偏 $\theta$，使船速的沿河岸分量抵消水流，合速度恰垂直對岸。兩圖均取 $v=5$、$u=3$（同例題），右圖 $\sin\theta=u/v=3/5$、$\theta=37^\circ$。}
...
\subsection*{\sffamily D.\ 風中飛機：同一招}
...
\item \textbf{平面相對速度}：$\vec v_{B/A}=\vec v_B-\vec v_A$（向量相減，分量各自算）；對地＝對介質＋介質對地。
  \begin{itemize}\setlength{\itemsep}{1pt}
  \item \textbf{一般情形}（兩物體沿不同方向動，如雨中行人）：直接把兩個對地速度向量相減，分量各算再合成。
  \item 行船／飛機（兩已知向量垂直的特例）：船頭直指對岸時\textbf{渡河時間 $t=\dfrac{d}{v}$ 與水流無關}，被沖到下游 $ut$ 處。
