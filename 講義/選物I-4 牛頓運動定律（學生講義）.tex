...
\begin{補充框}{名詞：正向力（normal force）$N$}
兩物體接觸時，接觸面為阻止彼此擠入對方，會沿\textbf{垂直接觸面、指向物體外側}的方向互相推擠，這個力稱為\textbf{正向力 $N$}（normal 即「垂直」之意）。它是自由體圖上最常出現的接觸力，使用前須先掌握三件事：
\begin{itemize}\setlength{\itemsep}{1pt}
\item \textbf{方向}永遠\textbf{垂直於接觸面}（水平地面上鉛直向上、斜面上垂直斜面向外），「垂直接觸面」與「鉛直」並非同義。
\item \textbf{大小不固定、也不必等於 $mg$}：$N$ 是由「物體沿垂直接觸面方向既不穿入、也不跳離」這個限制決定的\textbf{未知量}，須由該方向的 $\sum F=ma$ 解出。
\item 物體一旦離開接觸面（被拋起、自由落體），就沒有正向力。
\end{itemize}
\end{補充框}
...
\begin{補充框}{滑動的臨界角 $\theta_c$：$\tan\theta_c=\mu_s$}
把物體放在傾角可調的斜面上，從水平\textbf{緩緩加大傾角}。傾角越大，沿斜面下滑的驅動力 $mg\sin\theta$ 越大，而正向力 $mg\cos\theta$ 越小、最大靜摩擦 $\mu_s\,mg\cos\theta$ 隨之變小。當傾角加大到\textbf{剛要下滑}的\textbf{臨界角（critical angle）$\theta_c$} 時，靜摩擦恰好達到上限，沿斜面方向平衡：
$$mg\sin\theta_c = \mu_s N = \mu_s\, mg\cos\theta_c.$$
兩邊消去 $mg$，得
$$\boxed{\;\tan\theta_c = \mu_s\;}$$
只要量出物體剛要下滑時的傾角，即可定出靜摩擦係數 $\mu_s$；此結果與物體的質量無關。這是一個課堂上即可進行的測量。
\end{補充框}
...
\fig{si4-apparent-weight}{0.96}{圖 4-4：電梯中的視重。四種狀態下，重力 $mg$（紅，向下）大小不變；磅秤讀數即地板對人的正向力 $N$（藍，向上）隨加速度 $a$ 改變：靜止／等速時 $N=mg$、向上加速時 $N>mg$（變重）、向下加速時 $N<mg$（變輕）、自由下墜時 $N=0$（失重）。}

由 $N=m(g+a)$ 可看出：式中\textbf{變的只有加速度 $a$}，$m$ 與 $g$ 都沒變。換言之，\textbf{視重的本質就是正向力 $N$}，「體重變化」的根源是\textbf{加速度}，而非重力或質量改變。此一道理也不限於鉛直方向：人在水平加速的車中、或靠著傾斜椅背時，座椅對人的作用力同樣會偏離 $mg$；只是高中題目以鉛直電梯為標準題型。
...
\fig{si4-fbd-incline}{0.66}{圖 4-5：斜面上物體的自由體圖。坐標軸沿斜面與垂直斜面，重力 $mg$ 分解為沿斜面向下的 $mg\sin\theta$ 與垂直壓向斜面的 $mg\cos\theta$；$N$ 為正向力，$f$ 為摩擦力。}

傾角 $\theta$ 時，重力 $mg$（鉛直向下）分解為：
$$\underbrace{mg\sin\theta}_{\text{沿斜面向下}},\qquad \underbrace{mg\cos\theta}_{\text{垂直壓向斜面}}.$$
垂直斜面方向無加速度，故 $N = mg\cos\theta$。據此，含摩擦時最大靜摩擦為 $f_{s,\max}=\mu_s N=\mu_s\, mg\cos\theta$，可與沿斜面的驅動力 $mg\sin\theta$ 比較，判斷物體會不會下滑——\textbf{含摩擦的斜面有兩種結局，須先分類}：
\begin{itemize}\setlength{\itemsep}{1pt}
\item $mg\sin\theta \le f_{s,\max}$：\textbf{物體不下滑}（靜止平衡）。此時靜摩擦恰好抵消下滑分量，$f_s=mg\sin\theta$（\textbf{不是} $\mu_s N$；只有剛要下滑的臨界瞬間才達上限）。
\item $mg\sin\theta > f_{s,\max}$：\textbf{物體下滑}。摩擦改為動摩擦 $f_k=\mu_k N$，再由沿斜面 $\sum F=ma$ 求加速度，$ma=mg\sin\theta-f_k$。
\end{itemize}
務必避免一個常見錯誤：把靜止在斜面上、未下滑物體的靜摩擦直接寫成 $\mu_s N$；未下滑時靜摩擦只出力到「剛好不動」，其值為 $mg\sin\theta$，通常小於上限。

\subsection*{\sffamily E.\ 連接體}

兩個以上的物體用繩相連、跨過滑輪時，稱為\textbf{連接體}。

\fig{si4-connected}{0.66}{圖 4-6：連接體。桌面上的 $m_1$ 經理想滑輪以輕繩連到懸掛的 $m_2$。兩物體共用同一加速度 $a$，繩中張力 $T$ 在兩物體的方程式中皆出現、方向相反。}
...
求加速度時也可用較快的\textbf{整體法}：把整個系統當作一個總質量，以「驅動力 $-$ 阻力」除以總質量直接得 $a$（系統內部的張力成對抵消，不必計入）。但要注意：\textbf{整體法只能求出 $a$，求不出張力}。任何「物體之間互相作用的力」（張力或下面的接觸力），一律要回到\textbf{隔離法}——單獨對其中一個物體列式——才能求得。

\begin{補充框}{接觸推擠型連接體：用隔離法求物體間的接觸力}
連接體不一定靠繩相連，也可能是兩物體\textbf{直接接觸、互相推擠}（例如兩塊木塊前後緊貼、一起被推動，或多本書相疊一起被推）。此時把它們連在一起的不再是張力，而是一對沿接觸面的\textbf{接觸力（contact force）}，正是第三定律的現場：前塊推後塊多大力，後塊就反推前塊多大力。

求接觸力的標準作法分兩步：
\begin{enumerate}\setlength{\itemsep}{1pt}
\item \textbf{整體法求 $a$}：把所有物體當成一個總質量，只用外加推力 $F$ 除以總質量，得共同加速度 $a$。
\item \textbf{隔離法求接觸力}：單獨取出\textbf{被推的那一塊}畫自由體圖，沿運動方向只有「接觸力」在推它，故\textbf{接觸力 $=m_{\text{被推塊}}\times a$}。
\end{enumerate}

\fig{si4-contact-push}{0.82}{圖 4-7：接觸推擠型連接體。上排為整體情境：外力 $F$ 推後塊 $m_1$，$m_1$ 再推前塊 $m_2$，兩塊以同一加速度 $a$ 前進。下排為前塊 $m_2$ 的隔離自由體圖：水平方向只有 $m_1$ 對它的接觸力 $F_{1\to 2}$，故 $F_{1\to 2}=m_2\,a$，一般小於外力 $F$。}

由此可知一個關鍵事實：\textbf{接觸力一般不等於外加推力 $F$}。接觸力只負責加速「它所推的那一塊」，因此等於那一塊的質量乘以共同加速度，通常小於 $F$。同一組物體，從不同一端施力，物體間的接觸力也會不同（被推著前進的總質量不同）。
\end{補充框}
...
\item 「兩塊相貼被推動時，接觸力等於外加推力 $F$」並不正確。接觸力只負責加速被它推的那一塊，等於該塊質量乘以共同加速度，一般小於 $F$。
\end{itemize}
...
\item \textbf{視重}：$N=m(g+a)$，本質是磅秤對人的正向力 $N$；重力 $mg$ 不變，變的是加速度 $a$（向上加速變重、向下加速變輕、自由落體失重）。
\item \textbf{斜面}：坐標軸轉成沿斜面／垂直斜面，重力分解為 $mg\sin\theta$、$mg\cos\theta$。
\item \textbf{含摩擦}：先比較「驅動力」與 $f_{s,\max}=\mu_s N$ 判斷動不動，再選用 $f_s$ 或 $f_k$。未滑動時 $f_s$ 由合力反推（不等於 $\mu_s N$）；摩擦力與接觸面積無關；剛要下滑的臨界角 $\tan\theta_c=\mu_s$。
\item \textbf{連接體}：繩不伸長則同一 $a$；各物體各列一式，聯立消 $T$ 求解。\textbf{接觸推擠型}同理：整體法求 $a$、隔離法求接觸力（接觸力 $=m_{\text{被推塊}}\,a\neq F$）。內力（張力／接觸力）一律靠隔離法。
\end{itemize}
\end{補充框}
