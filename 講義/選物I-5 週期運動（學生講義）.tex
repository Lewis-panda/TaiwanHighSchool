...
\fig{si5-energy}{0.82}{圖 5-3：簡諧運動的能量隨位置分佈。位能 $U=\tfrac12 kx^2$ 為上凹拋物線（端點最高、平衡點為零），動能 $K=\tfrac12 k(A^2-x^2)$ 為下凹拋物線（與 $U$ 相反），兩者在 $x=\pm A/\sqrt{2}$ 處各佔一半；任意位置兩者相加皆為定值 $E=\tfrac12 kA^2$（水平線），即總力學能守恆。}
...
\fig{si5-projection}{0.92}{圖 5-4：左——半徑 $A$、角速度 $\omega$ 的等速圓周，其在 $x$ 軸上的投影為 $x=A\cos\omega t$；右——投影隨時間描出餘弦曲線，一個來回對應圓周轉一圈，週期同為 $T=2\pi/\omega$。}
...
\fig{si5-pendulum}{0.56}{圖 5-5：單擺受力分解。重力沿擺線的分量 $mg\cos\theta$ 與張力相關，沿切線的分量 $mg\sin\theta$ 指向平衡位置，即為回復力；小角度時 $\sin\theta\approx\theta$，回復力近似正比於位移，故近似為簡諧。}
