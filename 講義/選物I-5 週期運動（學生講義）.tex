...
\fig{si5-xva}{0.86}{圖 5-3：簡諧的位置、速度、加速度對時間。$x$ 與 $a$ \textbf{反相}（一個到頂、另一個到谷，因 $a=-\omega^2 x$）；$v$ 比 $x$ \textbf{提前 $T/4$}。端點（$x=\pm A$）時 $v=0$、$|a|$ 最大；平衡點（$x=0$）時 $|v|$ 最大、$a=0$——「速度最大」與「加速度最大」發生在不同位置。}

把 $x$–$t$、$v$–$t$、$a$–$t$ 三圖疊起來看：$x$ 與 $a$ 反相，$v$ 比 $x$ 提前四分之一週期。由此可直接讀出「端點 $v=0$、$|a|$ 最大；平衡點 $|v|$ 最大、$a=0$」，這是學生最常混淆的兩個位置。
...
簡諧系統在振盪時，能量在\textbf{動能（kinetic energy）}與\textbf{位能（potential energy）}之間來回搬運，但\textbf{總力學能不變}。在使用 $\tfrac12 mv^2$、$\tfrac12 kx^2$ 之前，先把這幾個第一次出現的名詞講清楚；完整的「功與能量」屬於後續課程，這裡先給夠用的定義與來歷。

\begin{補充框}{動能、彈簧位能、力學能（用前先講清楚）}
\begin{itemize}\setlength{\itemsep}{2pt}
\item \textbf{做功（work）}：力 $F$ 沿其方向推動物體移動距離 $d$，就說這個力對物體做功 $W=Fd$（力與位移同向為正、反向為負）。做功是「能量被轉移」的量度。
\item \textbf{動能（kinetic energy）}：物體因運動而具有的能量。質量 $m$、速率 $v$ 的物體，動能為 $K=\tfrac12 mv^2$。它由「等加速度下合力所做的功」得出：由 $v^2=v_0^2+2ad$ 與 $W=Fd=mad$，可得 $W=\tfrac12 mv^2-\tfrac12 mv_0^2$，故把 $\tfrac12 mv^2$ 取名為動能（與速率\textbf{平方}成正比）。
\item \textbf{彈簧位能（elastic potential energy）}：物體因位置或形變而儲存、可再放出的能量。彈簧回復力 $F=-kx$ 隨伸長量線性變化，把彈簧從平衡拉伸到 $x$ 所做的功（等於 $F$–$x$ 圖下三角形面積 $\tfrac12\cdot x\cdot kx$）就存成彈簧位能 $U=\tfrac12 kx^2$。力是常數時功為 $Fd$（矩形）；力隨位置線性變化時，取平均力 $\tfrac12 kx$ 乘位移 $x$，因而多一個 $\tfrac12$。
\item \textbf{力學能（mechanical energy）}：動能與位能的總和 $E=K+U$。當系統只受彈力、重力這類「位能可記帳」的力、沒有摩擦等耗散時，力學能守恆——總量不變，只在動能與位能間轉換。
\end{itemize}
這幾個量在本章只需會用；嚴謹的功–能定理與力學能守恆推導屬於後續「功與能量」單元。
\end{補充框}

以水平彈簧為例，位能存在被壓縮或拉伸的彈簧裡：
...
\fig{si5-energy}{0.82}{圖 5-4：簡諧運動的能量隨位置分佈。位能 $U=\tfrac12 kx^2$ 為上凹拋物線（端點最高、平衡點為零），動能 $K=\tfrac12 k(A^2-x^2)$ 為下凹拋物線（與 $U$ 相反），兩者在 $x=\pm A/\sqrt{2}$ 處各佔一半；任意位置兩者相加皆為定值 $E=\tfrac12 kA^2$（水平線），即總力學能守恆。}
...
\fig{si5-projection}{0.92}{圖 5-5：左——半徑 $A$、角速度 $\omega$ 的等速圓周，其在 $x$ 軸上的投影為 $x=A\cos\omega t$；右——投影隨時間描出餘弦曲線，一個來回對應圓周轉一圈，週期同為 $T=2\pi/\omega$。}
...
\fig{si5-pendulum}{0.56}{圖 5-6：單擺受力分解。重力沿擺線的分量 $mg\cos\theta$ 與張力相關，沿切線的分量 $mg\sin\theta$ 指向平衡位置，即為回復力；小角度時 $\sin\theta\approx\theta$，回復力近似正比於位移，故近似為簡諧。}
...
\begin{補充框}{單擺的能量：位能換成「重力位能」}
5-2 C 的能量觀也適用單擺，差別只在\textbf{位能的種類}：彈簧存的是彈簧位能 $\tfrac12 kx^2$，單擺存的是\textbf{重力位能（gravitational potential energy）}。擺球擺到端點時抬高了 $h$，重力位能為 $U=mgh$，其中 $h=L(1-\cos\theta)$ 是擺球比最低點高出的高度（$mgh$ 即「把質量 $m$ 抬高 $h$、克服重力所做的功」）。同樣地，端點全是位能、最低點（平衡點）全是動能，總力學能守恆：
$$mgh=\tfrac12 m v_{\max}^2.$$
因此「動能與位能接力、總能守恆」是彈簧與單擺\textbf{共有}的結構，只是位能的算法不同（彈力 $\to\tfrac12 kx^2$，重力 $\to mgh$）。
\end{補充框}
