...
\textbf{面積要帶正負號：}上述「功 $=$ 面積」預設了力\textbf{沿運動方向}（$F>0$），圍出的面積在 $x$ 軸\textbf{上方}，記為\textbf{正功}。但變力常中途反向（先推後擋，或彈簧回彈時彈力與運動反向）：此時 $F<0$，曲線落到 $x$ 軸\textbf{下方}，那一塊面積要記為\textbf{負功}。因此變力作功嚴格說是\textbf{「帶正負號的面積」}——上方面積記正、下方面積記負，\textbf{淨功為兩者代數相加}。這與定力時「$\cos\theta$ 變號對應正功／負功」是同一回事，只是換用圖形面積來看；漏掉下方的負面積，是判讀 $F$–$x$ 圖最常見的錯誤。
...
\fig{t2-fx-signed-area}{0.74}{圖 2-3：$F$–$x$ 圖的面積要帶正負號。力沿運動方向（$F>0$）時面積在 $x$ 軸上方，記正功；力逆運動方向（$F<0$）時面積在 $x$ 軸下方，記負功。淨功為兩者代數相加。}
...
\fig{t2-conservative}{0.92}{圖 2-4：左——保守力（重力）做功與路徑無關，兩條不同路徑由 A 到 B 做的功相同，只取決於高度差；右——非保守力（摩擦）使力學能沿路徑遞減，遞減的部分轉成熱。}

\fig{t2-closed-loop}{0.82}{圖 2-5：保守力判準 2（封閉路徑做功為零）的圖解。物體沿矩形封閉迴路 $\text{P}\to\text{Q}\to\text{R}\to\text{S}\to\text{P}$ 繞一圈，重力在上升段做負功 $-mgh$、下降段做正功 $+mgh$、兩段水平段做零功；去程與回程的功大小相等、正負相消，總功為零。這正是「封閉路徑做功為零」與「路徑無關」等價的由來。}
...
\fig{t2-pe-sign}{0.66}{圖 2-6：重力位能可正可負。$U_g=mgh$ 中的 $h$ 是有正負的高度座標：物體在零點上方時 $h>0$、$U_g>0$；在零點下方時 $h<0$、$U_g<0$；在零點處 $U_g=0$。零點可任意選定，換零點只改變各處位能的數值，不改變有物理意義的位能差。}
...
\fig{t2-energy-conservation}{0.62}{圖 2-7：力學能守恆。高處位能大、動能小；谷底位能小、動能大；各處的總量 $K+U$ 相同。能量在 $K$ 與 $U$ 之間轉換，總和不變。}
