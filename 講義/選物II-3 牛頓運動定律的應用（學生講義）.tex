...
\textbf{「支點可任選」有一個前提：必須在合力已為零的情況下才成立。}「換一個支點，力矩和仍為零」這個方便性質，是建立在 $\sum\vec F=0$ 之上的；若合力不為零，換支點時力矩和會跟著改變，這條性質就失效。所以實務上仍須把兩個條件\textbf{都列齊}——先有 $\sum\vec F=0$，再用 $\sum\tau=0$——不能只憑一條力矩方程式取代整組平衡條件。
\end{補充框}
...
\begin{補充框}{受側向推力時：「先滑」還是「先翻」？}
上面的傾倒判準描述的是「靜靜放著、慢慢傾斜」的情形。但在真實情境（推櫃子、車輛轉彎受側向力）中，物體受到一個\textbf{水平推力 $F$} 時，有\textbf{兩種}可能的破壞方式，會先發生哪一種須\textbf{比較}才能判斷。設物體質量 $m$、底寬 $w$，推力作用在離地高 $h$ 處。

\begin{itemize}\setlength{\itemsep}{1pt}
\item \textbf{先滑動}：當 $F$ 超過最大靜摩擦 $\mu_s N$（此處 $N=mg$）時，物體整個滑開。門檻為 $F_{\text{滑}}=\mu_s mg$。
\item \textbf{先翻倒}：當 $F$ 對底邊（pivot）的力矩超過重力的回正力矩時翻倒。推力的力臂為 $h$、重力回正的力臂為 $w/2$，門檻為 $F_{\text{翻}}=mg\,\dfrac{w/2}{h}=\dfrac{mgw}{2h}$。
\end{itemize}

比較兩個門檻，哪個小就先發生哪一種：$F_{\text{滑}}<F_{\text{翻}}$（即 $\mu_s<\dfrac{w}{2h}$）時\textbf{先滑}；反之 $\mu_s>\dfrac{w}{2h}$ 時\textbf{先翻}。因此\textbf{地面越滑（$\mu_s$ 小）越傾向先滑；物體又高又窄、地面又粗糙（$\mu_s$ 大、$w/h$ 小）越傾向先翻}。「冰面上的高櫃推一下就滑開、地毯上的同一個高櫃推一下卻翻倒」正是同一條物理的兩種結局。只看「重心出不出界」會漏掉「其實它早就滑走了」這另一半。
\end{補充框}
...
\textbf{靜摩擦力的方向也會自動調整。}靜摩擦不只\textbf{大小}會自動調整，\textbf{方向}也會跟著外力的趨勢反轉——它永遠指向「阻止物體相對滑動」的那一側。同一個物體放在斜面上：沒有人推它時，靜摩擦朝\textbf{上}（頂住下滑的趨勢）；若用力把它往上推到「快要往上滑」時，靜摩擦反而朝\textbf{下}（阻止上滑）。這與動摩擦不同：動摩擦的方向由\textbf{實際的滑動方向}決定（恆與滑動方向相反），而靜摩擦的方向由\textbf{即將滑動的趨勢}決定，會隨外力反轉。這是 $f_s$ 與 $f_k$ 的關鍵差異。
...
\begin{補充框}{加了沿斜面向上的外力：臨界在「兩頭」}
上面只處理「重力自己把物體往下拉」的情形，靜摩擦只朝上頂。一旦在斜面上\textbf{再加一個沿斜面向上的外力 $F$}（例如拉繩、用手往上推），就會出現\textbf{兩個}臨界，須分兩邊判斷。關鍵是先想清楚物體「想往哪邊滑」，靜摩擦就指向反方向。

\begin{itemize}\setlength{\itemsep}{1pt}
\item \textbf{下滑臨界}（$F$ 太小，物體有往下滑的趨勢）：靜摩擦朝\textbf{上}頂，恰好將滑時
$$mg\sin\theta - F = \mu_s\,mg\cos\theta.$$
\item \textbf{上滑臨界}（$F$ 太大，物體有被推上去的趨勢）：靜摩擦\textbf{反過來朝下}拉，恰好將滑時
$$F - mg\sin\theta = \mu_s\,mg\cos\theta.$$
\end{itemize}

兩個臨界之間，物體\textbf{靜止不動}，其外力範圍為
$$mg\sin\theta-\mu_s\,mg\cos\theta \;\le\; F \;\le\; mg\sin\theta+\mu_s\,mg\cos\theta.$$
也就是說，在這個區間內，靜摩擦會自動調整\textbf{大小與方向}去維持平衡。判斷斜面滑不滑時，只算「往下滑」這一半，遇到「用力往上拉」的情形會全錯——\textbf{臨界其實在兩頭}，最大靜摩擦在兩種情形下分別指向相反方向。
\end{補充框}
